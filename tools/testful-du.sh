...
function instrumentProject() {
    ...
    then
	$JAVA -jar $BASE/tools/instrumenter.jar -dataFlowCoverage DU -dir $BASE/projects/$project -file $BASE/projects/$project/toInstrument.txt -dirInstrumented instrumented-$VERSION > $BASE/projects/$project/instrument-$VERSION.out 2> $BASE/projects/$project/instrument-$VERSION.err
    fi
}
